...
"""
    ...
    return safe_json_parse(res.output[0].content[0].text)
...
# 入力
text = st.text_area("メッセージ入力")

uploaded_file = None
if st.session_state.premium:
    uploaded_file = st.file_uploader("画像アップロード", type=["png","jpg","jpeg","webp"])
else:
    st.write("🔒 画像診断は有料版のみ")
